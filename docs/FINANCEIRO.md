...
| 2026-09-10 | **Sócios: prévia ao vivo** (commit `884985b`). `calcDreResumoMes()` em `financial-utils` — DRE enxuto compartilhado (mesmas regras/categorias do `DreCompetencia`). A tela `Socios` mostra o resultado ao vivo quando o mês não está fechado, pra o sócio ver a cota estimada; a distribuição de fato continua travada até o DRE fechar. Verificado no preview: agosto prévia +R$11.984, cota de cada sócio ~R$5.992; Pedro já "Repassado no mês R$7.460" (retiradas). |
| 2026-09-10 | **Config populado + Asaas backfill.** `balanco_config`: `saldo_inicial_contas` (09/09: inter 2620,94 / c6 2234,40 / mp 38,20 / asaas 508,56 / voomp 413,86), `saldo_caixa_manual` 5815,96, `taxas` = [voomp/boleto 5,9%/0 · asaas/boleto 0/1,99 · asaas/pix 0/1,99]. `conta_recebimento='asaas'` + `taxa_valor=1.99` em 7 pagamentos casados 1:1 com o extrato Asaas — de 1 para 8 dos ~110 com conta. Custo Espaço Certo (NPA #19) R$900; repasse Keila corrigido: **Voomp 5,9% (print do Pedro: R$6,48/R$109,90)** → 50% do líquido = R$51,71/parcela (ago 18× = R$930,78 + taxa Voomp R$116,64; set 1× R$51,71 + R$6,48). |
| 2026-09-10 | **DRE por sócio** (commit `c206ed2`). `calcDrePorSocio` em financial-utils aplica o modelo de divisão Pedro/Rodrygo (ver memória `divisao-turmas-e-socios` / Raio-X): mensalidades pelo motor de repasse (Onze Digital=Pedro, IDM=Rodrygo, Keila=investidor), eventos NPA 50/50, DKSoft 100% Rodrygo, Zaffalon 100% Pedro, PNL 50/50; custos fixos 50/50, professoras pela proporção de mensalidade, evento 50/50, taxa Voomp 100% Rodrygo. Tela Sócios mostra `receita dele − custo dele = cota` por card + % real do mês. Config `socios` = Pedro 30%/R$2.500, Rodrygo 70%/R$6.000 (o % vira contexto; a cota real vem do cálculo). `#02726/OnzeDS` ganhou o split Onze Digital 50% que faltava. `balanco_config.taxas` = Voomp 5,9%. Verificado no preview: ago empresa +R$12.026 → Pedro cota R$8.022 (41%) / Rodrygo R$11.414 (59%). |
| 2026-09-10 | **View de receita de eventos** (commit `3982202`, migration `20260910120000`). `vw_receita_eventos_mes` agrega ingressos (qtd×unit) + material + matrículas de `npa_evento_leads` por `data_evento`. `DreCompetencia` + prévia do `Socios` (`calcDreResumoMes` ganhou param `receitaEventosMes`) somam isso na Receita bruta como bloco "Eventos NPA / IDM pelo Brasil", ao vivo. As 5 linhas manuais de balanco_itens dos eventos ago/set foram zeradas. `types.ts` registrou a view. **Setembro reflete Curitiba (19/09) + SJC (26/09) automaticamente.** DRE ago (tela): receita bruta R$29.461, resultado **+R$12.026**; set parcial resultado −R$2.101. |
