-- Guarda o slug do plano (padrao/promo/997/15x50/...) direto no aluno, em
-- vez de tentar adivinhar "qual plano era" no contrato comparando
-- valor_mensalidade com numeros magicos (997/110). Essa deteccao por valor
-- ja gerou bug real duas vezes (2026-09-04, link /997) e ia quebrar nao vez
-- com o link /15x50 (R$50/parcela, nao bate com nenhum valor conhecido).
-- plano_slug vira a fonte de verdade -- autentique-criar usa ele pra puxar
-- avista/parcela certos da mesma tabela de planos usada no formulario.
ALTER TABLE public.alunos ADD COLUMN plano_slug text;
...
COMMENT ON FUNCTION public.matricula_time_comercial_criar(
  ...
  'Cria matricula (aluno) a partir da ficha publica /matricula/:vendedor (ou /promo, /997, /15x50...) do Time Comercial. SECURITY DEFINER. p_plano_slug e a fonte de verdade pro contrato saber qual menu de precos mostrar (evita detectar por valor magico).';
