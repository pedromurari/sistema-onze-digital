...
--
-- Estas sao as que varias telas com permissoes diferentes usam ao mesmo tempo. Mapeei
-- tela por tela antes de escrever (scripts/mapear-tabelas-para-telas.py) e achei tres
-- armadilhas que teriam quebrado producao num lote cego:
--   * `whatsapp_mensagens` alimenta o chat do Time Comercial, nao so a Cobranca.
--   * `lead_aquecimento_vendedores` e ESCRITA pelo ChatTimeComercial, nao so lida.
--   * `funnel_configs` e usada pelo NPAKanban alem do FunilLancamento.
-- Por isso varias entram com mais de um recurso aceito, em vez de um so.
--
-- Corrige tambem uma regressao introduzida em 20260821232214: `parceiros_links` tem
-- policy de leitura anonima (o redirecionador /ir/:slug), mas o revoke geral de GRANT do
-- anon derrubou o acesso mesmo com a policy no lugar. Policy sem GRANT nao adianta.

grant select on public.parceiros_links to anon;

-- ─── Helper que aceita varios recursos e preserva as policies anon ───────────
...
-- ─── Novo recurso: cadastro de alunos ────────────────────────────────────────
-- `alunos` e lida por 16 telas, de Financeiro a Time Comercial. Amarrar a `financeiro`
-- teria tirado a tela do vendedor; por isso ganha recurso proprio.
...
-- Vendedor mantem o que tem hoje: o Time Comercial depende de `alunos`.
insert into public.role_permissoes (papel, recurso, acao)
...
-- ─── `alunos` com escopo por turma, preservando a captura publica ────────────
do $$
declare r record;
begin
  for r in select policyname from pg_policies
...
-- ─── Tabelas filhas de aluno: mesma janela, via join ─────────────────────────
do $$
declare r record;
begin
...
-- ─── Disparos (AquecimentoLeads e aba do DisparosMonitor) ────────────────────
...
-- ─── Funil: compartilhado entre Lancamentos, NPA e Funil de Lancamento ───────
