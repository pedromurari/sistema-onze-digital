...
--
-- Problema do modelo antigo (`user_access_permissions`): cada tela nova exigia uma
-- coluna nova, uma migration, mais um campo em quatro lugares do TypeScript e mais um
-- switch no TeamManagement. Permissao virou schema, quando deveria ser dado.
--
-- Modelo novo: recurso (catalogo) x papel x acao, com override por pessoa.
--   app_recursos            — o que existe pra proteger
--   role_permissoes         — o que cada papel pode, por padrao
--   user_permissao_override — excecao pontual por pessoa (positiva ou negativa)
--
-- Esta migration NAO muda o acesso de ninguem. Ela reproduz o estado atual no modelo
-- novo, incluindo as excecoes ja cadastradas. Apertar de fato e a sprint 1.3.
--
-- `user_access_permissions` continua existindo: as listas `allowed_lancamento_ids` e
-- `allowed_financeiro_turma_ids` sao escopo por registro e serao tratadas na 1.3.
...
-- vendedor e parceiro: espelham DEFAULT_NON_ADMIN_PERMISSIONS de src/lib/access-control.ts.
-- Hoje `getDefaultPermissions` devolve o mesmo default pra qualquer papel != admin, entao
-- parceiro recebe exatamente o mesmo que vendedor — preservado aqui e sinalizado ao dono.
insert into public.role_permissoes (papel, recurso, acao)
...
-- professora (papel novo, ninguem usa ainda)
insert into public.role_permissoes (papel, recurso, acao)
select 'professora'::public.app_role, r.recurso, a.acao
from (values ('dashboard'),('operacoes'),('mapa_mental')) r(recurso)
cross join (values ('ver'),('editar')) a(acao);

-- ─── Excecoes ja cadastradas viram override ──────────────────────────────────
-- Le a linha atual de cada pessoa e grava override so onde ela DIVERGE do padrao do
-- papel. Admin fica de fora: hoje `canAccessView` retorna true antes de olhar as flags,
-- entao as flags de admin sao decorativas — copia-las viraria restricao que hoje nao existe.
with mapa(recurso, coluna) as (values
  ...
  -- as tres telas abaixo hoje sao amarradas a permissao de Cobranca em canAccessView
...
-- Uma chamada devolve a matriz inteira do usuario logado — substitui a leitura das
-- 20 colunas no AuthContext.
...
-- ─── RLS das proprias tabelas de permissao ───────────────────────────────────
