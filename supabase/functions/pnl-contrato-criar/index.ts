/**
 * pnl-contrato-criar
 * Gera e envia (Autentique + WhatsApp) o contrato de matrícula do PNL
 * (Practitioner/Master) -- venda negociada caso a caso pela Helen via
 * WhatsApp, cobrança feita por fora do sistema (link avulso de Mercado
 * Pago/Asaas). Chamado depois de `pnl_matricula_criar` já ter registrado o
 * aluno no CRM.
 *
 * Diferente de autentique-criar (contrato da Psicanálise): aqui não existe
 * um "menu de condições" fixo (a/b/c) porque o preço é negociado por
 * cliente, não um plano tabelado -- a cláusula 4 já nasce mostrando
 * diretamente a condição que foi fechada. Também não há parceria de
 * extensão universitária (só certificado do próprio Instituto
 * Despertamente) nem duração fixa em meses/horas (2026-09-04: dono do
 * produto confirmou que não tem esse dado fechado ainda).
 *
 * Body: { aluno_id, cpf, data_nascimento, endereco, cep, cidade_estado, enviar_wpp? }
 * (mesmo contrato de `alunos` já preenchido por pnl_matricula_criar --
 * busca produto/forma_pagamento/valor_mensalidade/total_mensalidades de lá.)
 */
...
function buildContratoHtml(d: Record<string, unknown>): string {
  ...
  const telefone = String(d.whatsapp || '');
  const endereco = String(d.endereco || '');
  const cep      = String(d.cep || '');
  const cidEst   = String(d.cidade_estado || '');
  const pais     = String(d.pais || 'Brasil');
  const produto  = String(d.produto || '');
  const formaPag = String(d.forma_pagamento || '').toLowerCase();
  const valorParcela = Number(d.valor_parcela || 0);
  const numParcelas  = Number(d.num_parcelas || 1);
  const diaVencimento = d.dia_vencimento != null ? String(d.dia_vencimento) : '';

  const nomeCurso = NOME_PRODUTO[produto] || 'PNL';
  const valorTotal = valorParcela * numParcelas;

  const isVista  = formaPag === 'avista';
  const isBoleto = formaPag === 'boleto';
  const isCartao = formaPag === 'cartao';
  ...
  if (isVista) {
    formaResumo = 'Pagamento à vista';
    condicaoTexto = `pagamento à vista, no valor de R$ ${fmt(valorTotal)}`;
  } else if (isCartao) {
    formaResumo = `Cartão de crédito ${numParcelas}x de R$ ${fmt(valorParcela)}`;
    condicaoTexto = `cartão de crédito em ${numParcelas}x (${numParcelas === 1 ? 'parcela única' : 'parcelas'}) de R$ ${fmt(valorParcela)}, totalizando R$ ${fmt(valorTotal)}, conforme disponibilidade e aprovação da operadora/meio de pagamento`;
  } else {
    const diaTxt = diaVencimento.replace(/\D/g, '') || diaVencimento || 'a combinar';
    formaResumo = `Boleto: ${numParcelas}x de R$ ${fmt(valorParcela)} — vencimento dia ${diaTxt}`;
    condicaoTexto = `boleto bancário em ${numParcelas} (${numParcelas === 1 ? 'parcela única' : 'parcelas mensais'}) de R$ ${fmt(valorParcela)}, totalizando R$ ${fmt(valorTotal)}, com vencimento todo dia ${diaTxt}`;
  }
  ...
  // Cláusula 6 -- cancelamento: mesma regra da CDC (7 dias de arrependimento,
  // universal, não depende do curso) + a especificidade de boleto (multa de
  // 1 parcela) que já existe no contrato da psicanálise, reaproveitada aqui.
  const clausula6Extra = (isVista || isCartao) ? `
...
<p class="c" style="margin-left:16px;">II – multa rescisória equivalente a 1 (uma) parcela do plano, atualmente no valor de R$ ${fmt(valorParcela)}.</p>
<p class="c">Após a efetivação do cancelamento, as parcelas vincendas deixarão de ser cobradas, ressalvadas obrigações já constituídas.</p>
`;
  const numClausula64 = isBoleto ? '6.4' : '6.5';
  const numClausula65 = isBoleto ? '6.5' : '6.6';
...
<table class="ficha">
  ...
  <tr>
    ...
    <td><span class="fl">Curso</span><span class="fv">${nomeCurso.toUpperCase()}</span></td>
  </tr>
  <tr>
    <td><span class="fl">Plano / Forma de Pagamento</span><span class="fv">${formaResumo}</span></td>
    <td colspan="2"><span class="fl">Valor Total</span><span class="fv">R$ ${fmt(valorTotal)}</span></td>
  </tr>
</table>
...
<p class="c"><strong>1.1.</strong> O presente contrato tem por objeto a prestação de serviços educacionais referentes à formação <strong>${nomeCurso}</strong>, conforme programa pedagógico, calendário e orientações disponibilizados pela CONTRATADA.</p>
...
<div class="clausula-titulo">Cláusula 2 – Da Formação e da Certificação</div><hr class="div">
<p class="c"><strong>2.1.</strong> A CONTRATADA prestará a formação de acordo com sua proposta pedagógica, programa acadêmico e condições informadas ao CONTRATANTE no momento da matrícula.</p>
<p class="c"><strong>2.2.</strong> O certificado de conclusão será emitido pela própria CONTRATADA, mediante o cumprimento dos requisitos acadêmicos previstos na Cláusula 9.</p>

<div class="clausula-titulo">Cláusula 3 – Da Vigência e do Calendário</div><hr class="div">
<p class="c"><strong>3.1.</strong> A vigência acadêmica é aquela informada pela CONTRATADA no momento da matrícula, contada a partir da data de início da turma.</p>
...
<p class="c"><strong>4.1.</strong> O CONTRATANTE contratou, no ato da matrícula, a seguinte condição comercial: <strong>${condicaoTexto}</strong>.</p>
<p class="c"><strong>4.2.</strong> O valor e a forma de pagamento acima refletem a negociação comercial realizada com a CONTRATADA e constituem a condição efetivamente contratada, conforme registrado no quadro-resumo acima.</p>
...
${clausula6Extra}
<p class="c"><strong>${numClausula64}.</strong> A formalização do cancelamento poderá acarretar o encerramento do acesso às aulas, gravações, materiais, plataforma, grupos, bônus e demais recursos vinculados à matrícula.</p>
<p class="c"><strong>${numClausula65}.</strong> Nenhuma disposição desta cláusula limita direitos inderrogáveis assegurados ao consumidor pela legislação aplicável.</p>
...
serve(async (req) => {
  ...
  try {
    ...
    if (alunoErr || !aluno) {
      console.error('pnl-contrato-criar: aluno não encontrado', aluno_id, alunoErr);
    ...
    if (autentiqueToken && aluno.email) {
      try {
        const html = buildContratoHtml({
          ...
          valor_parcela: aluno.valor_mensalidade ?? 0,
          num_parcelas: aluno.total_mensalidades ?? 1,
          dia_vencimento: aluno.dia_vencimento ?? null,
        });

        const result = await criarDocumentoAutentique(
          autentiqueToken,
          `Contrato ${NOME_PRODUTO[aluno.produto as string] ?? 'PNL'} — ${aluno.nome}`,
          html,
          aluno.nome ?? '',
          aluno.email,
        );
        ...
        console.error('pnl-contrato-criar: Autentique error', e);
      }
    }
    ...
    if (enviar_wpp !== false && linkAssinatura && aluno.whatsapp) {
      try {
        ...
        console.error('pnl-contrato-criar: WPP error', e);
      }
    }
    ...
    console.error('pnl-contrato-criar error:', err);
