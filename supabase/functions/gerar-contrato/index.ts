...
async function enviarParaAutentique(d: Record<string, unknown>, html: string) {
  ...
  // não é o link público de assinatura (às vezes vem preenchido com uma URL
  // do painel administrativo autenticado, que dá "documento não existe" para
  // o signatário externo). O link público correto só existe através desta
  // mutation dedicada — por isso é sempre chamada, nunca reaproveitada.
  const link = await criarLinkAssinatura(assinatura.public_id);
